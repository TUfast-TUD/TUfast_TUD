window.onload = async function () {
  ...
  chrome.storage.local.get(
    ["dashboardDisplay", "saved_click_counter"],
    async function (result) {
      ...
      document.getElementById("saved_clicks").innerHTML =
        ...
        " Klicks</font> gespart: <a href='javascript:void(0)' id='time' target='_blank' style='color: purple;'>" +
        time +
        "</a></text>";
...
function openSettingsTimeSection() {
  chrome.runtime.sendMessage(
    { cmd: "open_settings_page", params: "time_settings" },
    function (result) {}
  );
  let isFirefox = navigator.userAgent.includes("Firefox/"); //attention: no failsave browser detection
  if (isFirefox) window.close();
  ...
  chrome.storage.local.get(["isEnabled"], function (resp) {
    chrome.runtime.sendMessage({ cmd: "get_user_data" }, function (result) {
      if (!(result.asdf === undefined || result.fdsa === undefined)) {
        ...
        chrome.runtime.sendMessage(
          { cmd: "open_settings_page", params: "auto_login_settings" },
          function (result) {}
        );
        window.close();
      }
