...
function copyStaticExtensionFiles() {
  return {
    ...
    writeBundle() {
      ...
      const buildsFirefox = process.env.TUFAST_BROWSER === 'firefox' || process.argv.includes('firefox')
      const defaultManifestName = buildsFirefox ? 'manifest.firefox.json' : 'manifest.chrome.json'
      const defaultManifestSource = path.join(srcDir, defaultManifestName)
