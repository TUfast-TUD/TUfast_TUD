...
console.log("Loaded TUfast browser addon");
...
function regAddContentScripts() {
  ...
  try {
    ...
    console.log("Error requesting additional content script for Chrome: " + e);
  }
  ...
  try {
    browser.contentScripts
      ...
      .then(() =>
        console.log("Successfully registered additional content scripts for FF")
      );
  } catch (e) {
    console.error("Error requesting additional content script for FF: " + e);
  }
}
...
chrome.storage.local.get(
  ...
  function (result) {
    if (
      !result.gotInteractionOnHostPermissionExtension1 &&
      result.saved_click_counter > 10
    ) {
  ...
  switch (reason) {
    ...
    case "update":
      ...
      chrome.storage.local.get(["dashboardDisplay"], (resp) => {
        if (
          resp.dashboardDisplay === null ||
          resp.dashboardDisplay === undefined ||
          resp.dashboardDisplay === ""
        ) {
          chrome.storage.local.set(
            { dashboardDisplay: "favoriten" },
            function () {}
      ...
      chrome.storage.local.get(["mostLiklySubmittedReview"], (resp) => {
        if (
          resp.mostLiklySubmittedReview === null ||
          resp.mostLiklySubmittedReview === undefined ||
          resp.mostLiklySubmittedReview === ""
        ) {
          chrome.storage.local.set(
            { mostLiklySubmittedReview: false },
            function () {}
      ...
      chrome.storage.local.get(["removedReviewBanner"], (resp) => {
        if (
          resp.removedReviewBanner === null ||
          resp.removedReviewBanner === undefined ||
          resp.removedReviewBanner === ""
        ) {
          chrome.storage.local.set(
      ...
      chrome.storage.local.get(["neverShowedReviewBanner"], (resp) => {
        if (
          resp.neverShowedReviewBanner === null ||
          resp.neverShowedReviewBanner === undefined ||
          resp.neverShowedReviewBanner === ""
        ) {
          chrome.storage.local.set(
            { neverShowedReviewBanner: true },
            function () {}
      ...
      chrome.storage.local.get(["seenInOpalAfterDashbaordUpdate"], (resp) => {
        if (
          resp.seenInOpalAfterDashbaordUpdate === null ||
          resp.seenInOpalAfterDashbaordUpdate === undefined ||
          resp.seenInOpalAfterDashbaordUpdate === ""
        ) {
          chrome.storage.local.set(
            { seenInOpalAfterDashbaordUpdate: 0 },
            function () {}
      ...
      chrome.storage.local.get(["flakeState"], function (result) {
        if (result.flakeState === undefined || result.flakeState === null) {
          //set to true, so that state will be false!
          chrome.storage.local.set({ flakeState: false }, function () {});
        }
      });
      ...
      chrome.storage.local.get(["showedFirefoxBanner"], function (result) {
        if (
          result.showedFirefoxBanner === undefined ||
          result.showedFirefoxBanner === null
        ) {
          chrome.storage.local.set(
            { showedFirefoxBanner: false },
            function () {}
          );
        }
      });
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  switch (request.cmd) {
    ...
    case "open_shortcut_settings":
      let isChrome = navigator.userAgent.includes("Chrome/"); // FIXME: attention: no failsave browser detection | also for new edge!
      let isFirefox = navigator.userAgent.includes("Firefox/"); // attention: no failsave browser detection
      if (isFirefox) {
        chrome.tabs.create({
          url:
            "https://support.mozilla.org/de/kb/tastenkombinationen-fur-erweiterungen-verwalten",
        });
      } else {
...
function save_clicks(counter) {
  ...
  chrome.storage.local.get(["saved_click_counter"], (result) => {
    saved_clicks =
      result.saved_click_counter === undefined ? 0 : result.saved_click_counter;
    chrome.storage.local.set(
      { saved_click_counter: saved_clicks + counter },
      function () {
...
async function setUserData(userData) {
  ...
  userDataEncrypted = userDataEncrypted
...
async function getUserData() {
  return new Promise(async (resolve, reject) => {
    ...
    chrome.storage.local.get(["Data"], async (Data) => {
      ...
      let iv = await Data.Data.slice(0, 32)
        .match(/.{2}/g)
        .map((byte) => parseInt(byte, 16));
      iv = new Uint8Array(iv);
      let userDataEncrypted = atob(Data.Data.slice(32));
      userDataEncrypted = new Uint8Array(
        userDataEncrypted.match(/[\s\S]/g).map((ch) => ch.charCodeAt(0))
      );
...
function saveCourses(course_list) {
  ...
  switch (course_list.type) {
    case "favoriten":
      chrome.storage.local.set(
        { favoriten: JSON.stringify(course_list.list) },
        function () {}
      );
    ...
    case "meine_kurse":
      chrome.storage.local.set(
        { meine_kurse: JSON.stringify(course_list.list) },
        function () {}
      );
