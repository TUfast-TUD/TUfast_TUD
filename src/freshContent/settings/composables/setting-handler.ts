...
const opalPdf = async (verb: Verbs, option?: OptionsOpalPdf): Promise<ResponseOpalPdf | boolean> => {
  ...
  if (verb === 'check') { return await sendChromeRuntimeMessage({ cmd: `${verb}_opalpdf_status` }) as ResponseOpalPdf } else { return await sendChromeRuntimeMessage({ cmd: `${verb}_opalpdf_${option}` }) as boolean }
}

const owa = async (verb: Verbs, option?: OptionsOWA): Promise<ResponseOWA | boolean> => {
  console.log(`${verb}_owa_${option}`)
  if (verb === 'check') { return await sendChromeRuntimeMessage({ cmd: `${verb}_owa_status` }) as ResponseOWA } else { return await sendChromeRuntimeMessage({ cmd: `${verb}_owa_${option}` }) as boolean }
}
