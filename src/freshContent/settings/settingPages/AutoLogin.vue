<template>
  ...
  <h3 class="onboarding-hide onboarding-hide-otp" style="margin-top: 4px">
    <span :class="`state ${currentLogin.state ? 'state--active' : 'state--inactive'}`"
      >{{ currentLogin.state ? strings.settings.pages.autoLogin.saved : strings.settings.pages.autoLogin.notSaved }}
    </span>
  </h3>
  <p class="max-line p-margin onboarding-hide-otp" style="margin-bottom: 16px">
    {{ strings.settings.pages.autoLogin.intro(currentLogin.name) }}
  </p>
  ...
  <div class="form onboarding-hide-otp">
    <div>
      ...
      <CustomInput
        ...
        :placeholder="loginDataSaved ? strings.common.saved : currentLogin.usernamePlaceholder"
    ...
    <div>
      ...
      <CustomInput
        ...
        :placeholder="loginDataSaved ? strings.common.saved : currentLogin.passwordPlaceholder"
        type="password"
    ...
    <CustomButton
      :title="strings.settings.pages.autoLogin.saveLogin"
      :disabled="!(passwordValid && usernameValid) || loginDataSaved"
      @click="submitSave"
    />
    <CustomButton
      :title="strings.settings.pages.autoLogin.deleteLogin"
      class="button--warn"
  ...
  <div v-if="currentLogin2FA" class="onboarding-hide">
    <h3 class="card-body-title onboarding-hide-otp" style="margin-top: 4rem">
      {{ strings.settings.pages.autoLogin.twoFactorTitle }}
    </h3>
    <h3 class="onboarding-hide onboarding-hide-otp" style="margin-top: 4px">
      <span :class="`state ${totpSecretSaved ? 'state--active' : 'state--inactive'}`"
        >{{ totpSecretSaved ? strings.settings.pages.autoLogin.saved : strings.settings.pages.autoLogin.notSaved }}
      </span>
    </h3>

    <p class="max-line p-margin">
      {{ strings.settings.pages.autoLogin.twoFactorHelp }}
      <span
        style="
          margin-top: 8px;
          margin-bottom: 8px;
    ...
    <a href="https://github.com/TUfast-TUD/TUfast_TUD/blob/main/docs/2FA.md" target="_blank">{{
      strings.settings.pages.autoLogin.twoFactorLink
    }}</a
    ><br /><br />
    <div class="form">
      <div>
      ...
      <CustomInput
        ...
        :placeholder="totpSecretSaved ? strings.common.saved : currentLogin2FA.totpSecretPlaceholder"
        :error-message="currentLogin2FA.totpSecretError"
        :disabled="totpSecretSaved"
        warn
      />
      <CustomButton
        :title="strings.settings.pages.autoLogin.saveKey"
        :disabled="!totpSecretValid || totpSecretSaved"
        @click="submitSaveTotp"
      />
      <CustomButton
        :title="strings.settings.pages.autoLogin.deleteKey"
        class="button--warn"
        style="min-width: 300px"
        :disabled="!totpSecretSaved"
