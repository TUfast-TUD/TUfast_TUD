...
export async function disableOpalInline () {
  ...
  if (chrome.declarativeNetRequest) await chrome.declarativeNetRequest.updateDynamicRules({ removeRuleIds: rules.map(r => r.id) })
  // As declarativeNetRequest is not supported as optional permission in Manifest V3, we don't need to remove it
  await (chrome.permissions as any).remove({ permissions: ['declarativeNetRequestWithHostAccess'] }).catch(() => { /* ignore */ })
}
...
export async function enableOpalHeaderListener () {
  const granted = await (chrome.permissions as any).contains({ permissions: ['declarativeNetRequestWithHostAccess'] })
  if (!granted) return
