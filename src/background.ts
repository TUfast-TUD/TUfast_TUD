...
async function openCourseLinks(links: string, behavior: string): Promise<void> {
  ...
  if (courseLinks.length > 25) {
    const linkType = links === 'favoriten' ? strings.content.background.favorites : strings.content.background.courses
    ...
    if (behavior === 'immediate_active') {
      ...
      if (currentTab?.id) {
        chrome.scripting.executeScript({
          ...
          args: [strings.content.background.tooManyLinks(linkType)]
        })
      }
    } else {
