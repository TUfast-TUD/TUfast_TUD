...
export function rebuildGraphFields(nodes: OpalSearchNode[]): OpalSearchNode[] {
  const byId = new Map(nodes.map((node) => [node.id, { ...node, childrenIds: [] as string[] }]))
...
export function buildGraphNode(
  ...
  const course = path.find((entry) => entry.type === 'course') || node
  ...
  // Keep title and parent numbers separate: "20. Uebungsblatt" should beat
  // "1. Uebungsblatt (20. bis ...)" when the query asks for sheet 20.
