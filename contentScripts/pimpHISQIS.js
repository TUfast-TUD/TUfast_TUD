...
function getGradesFromTable() {
    ...
    tableRows.forEach((row) => {
        ...
        [...row.cells].forEach((table_data) => {
            if (table_data.lastElementChild === null) {
                new_row.push(table_data.innerHTML.trim());
            } else {
                new_row.push(table_data.lastElementChild.innerHTML.trim());
            }
        });
        table.push(new_row);
    });
    ...
    table.filter((row, index) => row[0][1] === '0' || parseInt(row[0]) < 1000 ? mainLevel.push(index) : []);
    table.filter((row, index) => row[0][3] === '0' && mainLevel.indexOf(index) < 0 ? moduleLevel.push(index) : []);
    table.filter((row, index) => (mainLevel.indexOf(index) && moduleLevel.indexOf(index)) < 0 ? examLevel.push(index) : []);
